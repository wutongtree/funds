$(function(){
    ...
    $('#login-form').validate({
        ...
        submitHandler:function(form) {
            ...
            $(form).ajaxSubmit({
                ...
                success:function(data) {
                    if (data.code) {
                       setTimeout(function(){window.location.href="/my/funds"}, 0);
                    } else {
	...
	$('#createfund-form').validate({
        ...
        submitHandler:function(form) {
            ...
            $(form).ajaxSubmit({
                ...
                success:function(data) {
                    if (data.code) {
                       setTimeout(function(){window.location.href="/funds/manage"}, 0);
                    } else {
                       dialogInfo(data.message)
                       setTimeout(function(){ $('#dialogInfo').modal('hide'); }, 1000);
                    }
	...
	$('#setfundnetvalue-form').validate({
        ignore:'',
        submitHandler:function(form) {
            var url = '/fund/setfundnetvalue';
            $(form).ajaxSubmit({
                url:url,
                type:'POST',
                dataType:'json',
	...
	$('#setfundthreshhold-form').validate({
        ignore:'',
        submitHandler:function(form) {
            var url = '/fund/setfundthreshhold';
            $(form).ajaxSubmit({
                url:url,
                type:'POST',
                dataType:'json',
	...
	$('#buyfund-form').validate({
        ignore:'',
        rules : {
            buycount:{ required: true}
        },
        messages : {
            buycount : {required: '请填写金额'}
        },
        submitHandler:function(form) {
            var url = '/my/buyfund';
            $(form).ajaxSubmit({
	...
	$('#redeemfund-form').validate({
        ignore:'',
        rules : {
            redeemcount:{ required: true}
        },
        messages : {
            redeemcount : {required: '请填写份额'}
        },
        submitHandler:function(form) {
            var url = '/my/redeemfund';
            $(form).ajaxSubmit({
                url:url,
                type:'POST',
                dataType:'json',
